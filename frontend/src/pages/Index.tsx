    ...
    <div className="max-w-7xl mx-auto space-y-8">
        ...
        <section className="text-center py-8 md:py-12">
          <h1 className="font-display text-3xl md:text-5xl font-extrabold text-foreground mb-3">
            India's Complete <span className="text-primary">Mandi</span> Intelligence
          </h1>
          <p className="text-muted-foreground text-base md:text-lg max-w-2xl mx-auto mb-8">
            Real-time prices from all 7,021 APMCs — eNAM, Agmarknet & State Portals unified in one platform.
          </p>
          ...
          <div className="max-w-xl mx-auto flex gap-2">
            ...
            <Button size="lg" className="h-12 px-6" asChild>
              <Link to={`/dashboard${search ? `?q=${encodeURIComponent(search)}` : ""}`}>
        ...
        {statsLoading ? (
          ...
          <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard icon={BarChart3} label="Today's Updates" value={stats.todaysUpdates} />
            <StatCard icon={MapPin} label="States Covered" value={stats.statesCovered} />
          </section>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Card className="border-l-4 border-l-primary">
                <CardContent className="pt-6">
                  <p className="text-3xl font-bold font-display text-primary">{stats.enamIntegrated.toLocaleString()}</p>
                  <p className="text-sm text-muted-foreground">eNAM Integrated</p>
                  <p className="text-xs text-muted-foreground/70 mt-1">
                    {((stats.enamIntegrated / stats.totalApmcs) * 100).toFixed(1)}% of total
                  </p>
                </CardContent>
              </Card>
              <Card className="border-l-4 border-l-secondary">
                <CardContent className="pt-6">
                  <p className="text-3xl font-bold font-display text-secondary">{stats.statePortalCovered.toLocaleString()}</p>
                  <p className="text-sm text-muted-foreground">State Portals</p>
                  <p className="text-xs text-muted-foreground/70 mt-1">
                    {((stats.statePortalCovered / stats.totalApmcs) * 100).toFixed(1)}% of total
                  </p>
                </CardContent>
              </Card>
              <Card className="border-l-4 border-l-destructive">
                <CardContent className="pt-6">
                  <p className="text-3xl font-bold font-display text-destructive">{stats.uncovered.toLocaleString()}</p>
                  <p className="text-sm text-muted-foreground">Uncovered</p>
                  <p className="text-xs text-muted-foreground/70 mt-1">
                    {((stats.uncovered / stats.totalApmcs) * 100).toFixed(1)}% gap remaining
                  </p>
                </CardContent>
              </Card>
            </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display text-xl font-bold">Top Gainers & Losers</h2>
              ...
              movers.slice(0, 8).map((m) => (
                <Card key={`${m.crop}-${m.state}`} className="hover:shadow-md transition-shadow">
                  <CardContent className="pt-4 pb-4">
                    ...
                    <Badge variant={m.direction === "up" ? "default" : "destructive"} className="text-xs">
                      {m.direction === "up" ? <TrendingUp className="h-3 w-3 mr-1" /> : <TrendingDown className="h-3 w-3 mr-1" />}
                      {m.changePercent > 0 ? "+" : ""}{m.changePercent}%
                    </Badge>
                  </div>
                  <p className="text-lg font-bold mt-2 font-display">₹{m.currentPrice.toLocaleString()}<span className="text-xs font-normal text-muted-foreground">/qtl</span></p>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        <section className="bg-primary/5 rounded-xl p-6 md:p-8 text-center border border-primary/10">
          <h2 className="font-display text-2xl font-bold mb-2">Explore Market Data</h2>
          <p className="text-muted-foreground mb-4">Dive into detailed prices, analytics, and geographic insights</p>
          <div className="flex flex-wrap gap-3 justify-center">
            <Button asChild><Link to="/dashboard">Market Prices</Link></Button>
            <Button variant="outline" asChild><Link to="/analytics">Analytics</Link></Button>
            <Button variant="outline" asChild><Link to="/map">Geographic Map</Link></Button>
          </div>
        </section>
    </div>
  );
