  ...
  useEffect(() => {
    api.getMyProfile()
      .then((data) => {
        ...
        setFarmSize(data.farmerDetails?.farmSize ? String(data.farmerDetails.farmSize) : "");
        setPrimaryCrops((data.farmerDetails?.primaryCrops || []).join(", "));

        setCompanyName(data.traderDetails?.companyName || "");
        setGstNumber(data.traderDetails?.gstNumber || "");
        setTradingStates((data.traderDetails?.tradingStates || []).join(", "));

        setOrganization(data.policyMakerDetails?.organization || "");
        setDesignation(data.policyMakerDetails?.designation || "");
        setPolicyFocusAreas((data.policyMakerDetails?.policyFocusAreas || []).join(", "));

        setStartupName(data.agriStartupDetails?.startupName || "");
        setStartupStage(data.agriStartupDetails?.stage || "idea");
        setStartupFocusAreas((data.agriStartupDetails?.focusAreas || []).join(", "));
      })
      .catch(() => {
        setProfile(null);
      })
